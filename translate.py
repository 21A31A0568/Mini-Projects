...
from googletrans import Translator #API Provides translation functionality
from gtts import gTTS #Google Text-to-Speech, is a Python library that converts text into audio files
import os #Used for system operations like creating directories and playing audio files.
...
target_language_input = input("Enter the target language (e.g., 'Hindi', 'Telugu', 'Tamil'): ").strip()# remove leading and trailing whitespace characters from a string
...
if target_language_input not in language_map:
    ...
    with mc as source: #Used to simplify exception handling
    ...
    if MyText:
        ...
        translator = Translator() #class in googletrans, is used to perform language detection and translation.

        # Detect the language of the recognized sentence
        detected_language = translator.detect(MyText).lang #The lang refers to the language code (ISO 639-1 code)
